...
定义N个数据拥有者$\{F_1...F_n\}$，都愿意贡献相应数据$\{D_1...D_n\}$共同训练一个机器学习模型。在传统的模式下，我们会把这些数据组合起来$D=D_1\cup ...\cup D_n$训练一个模型$M_{SUM}$。而在FL的机制中，数据拥有者可以在不暴露自己数据的情况下训练模型$M_{FED}$。除此之外，我们将$M_{FED}$这一模型的精度（记为$V_{FED}$）应该与$V_{SUM}$非常接近。设$\delta$是一个非负实数，若：$|V_{FED}-V{SUM}|<\delta$ ，我们说此FL算法具有$\delta $精度损失（$\delta-accuracy\  loss$）。
